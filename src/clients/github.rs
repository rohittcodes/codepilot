...
impl GitHubMCPClient {
    pub fn new(config: &Config) -> Self {
        Self {
            client: Client::new(),
            mcp_url: config.get_mcp_url("github").to_string(),
        }
    }
    ...
    pub async fn get_tools(&self) -> Result<Vec<Value>> {
        ...
        for attempt in 1..=3 {
            ...
            return Err(anyhow::anyhow!(
                "Failed to fetch tools from GitHub MCP: {}",
                response.status()
            ));
        }
        
        // If we get here, all attempts failed
        Err(anyhow::anyhow!("Failed to fetch tools from GitHub MCP after 3 attempts"))
    }
    ...
    pub async fn execute_tool(&self, tool_name: &str, arguments: Value) -> Result<Value> {
        ...
        Err(anyhow::anyhow!("Failed to execute tool on GitHub MCP: {}", response.status()))
    }
    ...
    pub async fn get_tool_schema(&self, tool_name: &str) -> Result<Value> {
        ...
        Ok(serde_json::json!({
            "name": tool_name,
            "description": "GitHub tool schema"
        }))
    }
