...
/// Linear MCP Client that connects to the authenticated Composio MCP server
...
impl LinearMCPClient {
    pub fn new(config: &Config) -> Self {
        Self {
            client: Client::new(),
            mcp_url: config.get_mcp_url("linear").to_string(),
        }
    }

    /// Get available tools from the Linear MCP server
    pub async fn get_tools(&self) -> Result<Vec<Value>> {
        // Add retry logic for rate limiting
        for attempt in 1..=3 {
            // MCP servers typically use POST requests with specific JSON-RPC format
            ...
            return Err(anyhow::anyhow!(
                "Failed to fetch tools from Linear MCP: {}",
                response.status()
            ));
        }
        
        // If we get here, all attempts failed
        Err(anyhow::anyhow!("Failed to fetch tools from Linear MCP after 3 attempts"))
    }

    /// Execute a tool via the Linear MCP server
    pub async fn execute_tool(&self, tool_name: &str, arguments: Value) -> Result<Value> {
        ...
        if response.status().is_success() {
            ...
            Ok(serde_json::json!({
                "status": "success",
                "message": format!("Executed tool: {}", tool_name),
    ...
    pub async fn get_tool_schema(&self, tool_name: &str) -> Result<Value> {
        let response = self
            .client
            .get(&format!("{}/tools/{}", self.mcp_url, tool_name))
            .send()
            .await?;
    ...
    /// List all available Linear operations
