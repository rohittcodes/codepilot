...
use crate::clients::GitHubMCPClient;
...
impl GitHubAgent {
    pub async fn new(api_key: String, config: &Config) -> Result<Self> {
        let github_client = GitHubMCPClient::new(config);
        
        // Get dynamic tools from MCP server
        let tools_response = github_client.get_tools().await?;
...
- 'I would use GITHUB_LIST_PROJECTS to show your projects'
...
Remember: ONLY use tools from the list above. Never use any other tools.",
    ...
    /// Process a natural language query and execute GitHub operations
        ...
        match llm_response {
            Ok(Ok(response)) => {
                if response.trim().is_empty() {
                    ...
                    Ok("I couldn't understand your request. Please try asking about GitHub repositories, issues, pull requests, or other GitHub operations.".to_string())
                } else {
                    // LLM provided reasoning - use it to guide tool selection
                    let result = self.try_execute_github_operation_with_llm_guidance(query, &response).await?;
                    Ok(format!("LLM Analysis: {}\n\nGitHub Operation: {}", response, result))
                }
            }
            Ok(Err(_)) => {
    ...
    async fn try_execute_github_operation_with_llm_guidance(&self, query: &str, llm_guidance: &str) -> Result<String> {
        ...
        if best_tool.is_none() {
            return self.try_execute_github_operation(query).await;
        }
        ...
        match self.github_client.execute_tool(&tool.name, arguments).await {
            Ok(result) => {
    ...
    /// Try to find and execute a relevant GitHub operation based on the query using dynamic tools
    async fn try_execute_github_operation(&self, query: &str) -> Result<String> {
        let query_lower = query.to_lowercase();
        ...
        Ok(format!(
            "No relevant GitHub tool found for your query: '{}'\n\nAvailable tools (showing top 5):\n{}",
            query,
            tool_names.join("\n")
        ))
    }
    ...
    fn calculate_tool_relevance_score(&self, query_lower: &str, tool: &ToolInfo) -> f32 {
        ...
        let patterns = vec![
            ...
            ("repository", vec!["repo", "repository", "project"], 0.8),
            ("issue", vec!["issue", "ticket", "bug", "problem"], 0.8),
            ("pull", vec!["pull", "pr", "merge", "request"], 0.8),
            ("commit", vec!["commit", "push", "code"], 0.8),
        ];
    ...
    fn create_dynamic_arguments(&self, tool_name: &str, _input_schema: &Value, query: &str) -> Value {
        ...
        if tool_name_lower.contains("list") && tool_name_lower.contains("repo") {
            // List repositories - typically needs pagination parameters
            serde_json::json!({
                "type": "all",
                "per_page": 10,
                "sort": "updated"
        ...
        } else if tool_name_lower.contains("list") && tool_name_lower.contains("issue") {
            // List issues
                serde_json::json!({
                    "state": "open",
                    "per_page": 10
                })
        } else if tool_name_lower.contains("list") && tool_name_lower.contains("pull") {
            // List pull requests
                serde_json::json!({
                "state": "open",
                    "per_page": 10
                })
        } else if tool_name_lower.contains("create") && tool_name_lower.contains("pull") {
            // Create pull request
                serde_json::json!({
                "title": "PR from AI Agent",
                "body": query,
    ...
    pub async fn test_connection(&self) -> Result<String> {
        match self.github_client.get_tools().await {
            Ok(tools) => {
        Ok(format!("GitHub MCP connection successful! Found {} tools", tools.len()))
            }
            Err(e) => {
                Ok(format!("GitHub MCP connection failed: {}", e))
            }
        }
    }
} 